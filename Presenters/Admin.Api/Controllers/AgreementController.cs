...
namespace Admin.Api.Controllers
{
    ...
    public class AgreementController : BaseController
    {
        ...
        public AgreementController(ILogger<AgreementController> logger, IAgreementService AgreementService)
        {
            _logger = logger;
            _AgreementService = AgreementService;
            _logger.LogInformation("AgreementController Initialized");
        }
        ...
        public async Task<ApiResponse<IEnumerable<Agreement>>> GetAll()
        {
            try
            {
               // var headerData = GetHeaderData();

                var result = await _AgreementService.GetAllAsync();
                return new ApiResponse<IEnumerable<Agreement>>()
                {
            ...
            catch (Exception ex)
            {
                ...
                return new ApiResponse<IEnumerable<Agreement>>() { Status = EnumStatus.Error, Message = ex.Message };
            }
        }
        ...
        public async Task<ApiResponse<Agreement>> GetById(ValueRequest request)
        {
            try
            {
                var result = await _AgreementService.GetByIdAsync(request.Id);
                return new ApiResponse<Agreement>()
                {
                    Status = EnumStatus.Success,
                    Data = result,
                };
            ...
            {
                _logger.LogError(ex.Message);
                return new ApiResponse<Agreement>() { Status = EnumStatus.Error, Message = ex.Message };
            }
        }
        ...
        public async Task<ApiResponse<bool>> Create(Agreement Agreement)
        {
            try
            {
                var result = await _AgreementService.CreateAsync(Agreement);
        ...
        public async Task<ApiResponse<bool>> Update(Agreement Agreement)
        {
            try
            {
                var result = await _AgreementService.UpdateAsync(Agreement);
                return new ApiResponse<bool>()
                {
                    Status = EnumStatus.Success,
                    Data = result,
        ...
        public async Task<ApiResponse<bool>> Delete(DeleteRequest request)
        {
            try
            {
                var result = await _AgreementService.DeleteAsync(request);
                return new ApiResponse<bool>()
                {
                    Status = EnumStatus.Success,
                    Data = result,
        ...
        public IActionResult GetAllExport()
        {
            try
            {
                var result = _AgreementService.GetAllAsync();
                var res = result.Result.ToList();
                List<Agreement> data = new();
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    {
                        var worksheet = workbook.Worksheets.Add("Agreement");
